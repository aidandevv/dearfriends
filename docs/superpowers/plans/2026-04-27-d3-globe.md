...
**Architecture:** A `GlobePanel` client component renders a D3 `geoOrthographic` SVG globe and is dynamically imported (SSR disabled) in the dashboard server component. Geocoding runs server-side in the `upsertContact` action via the Google Geocoding API; coordinates are stored as `lat`/`lng` on the `contacts` table and used only when present (graceful nulls for pre-existing contacts).

**Tech Stack:** `d3-geo`, `topojson-client`, Supabase PostgreSQL, Google Geocoding API, Next.js dynamic imports, React `useEffect` + `requestAnimationFrame`.
...
- [ ] **Update `upsertContact` to geocode and write back coords**
...
- [ ] **Install D3 and TopoJSON packages**
...
- [ ] **Add the dynamic import at the top of `app/dashboard/page.tsx`**
...
- [ ] **Replace the static SVG map with GlobePanel**
...
- [ ] **Also update the header subtitle to show geocoded pin count**
